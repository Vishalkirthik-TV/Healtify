...
export const WebSocketProvider: React.FC<WebSocketProviderProps> = ({
  children,
  serverUrl = process.env.NEXT_PUBLIC_WS_URL || 'ws://localhost:8000/ws/'
}) => {
  ...
  >(null);
  const animationReceivedCallbackRef = useRef<
    ((gesture: string) => void) | null
  >(null);
  const captionReceivedCallbackRef = useRef<((text: string) => void) | null>(
    null
  );

  const connect = useCallback(
    async (userId: string = 'test-client') => {
            ...
            if (data.status === 'connected') {
              ...
              if (data.message && data.message.startsWith('Calling')) {
                ...
                if (match) {
                  ...
                  import('../utils/TwilioVoiceManager').then(
                    ({ default: TwilioVoiceManager }) => {
  ...
  const onAnimationReceived = useCallback(
