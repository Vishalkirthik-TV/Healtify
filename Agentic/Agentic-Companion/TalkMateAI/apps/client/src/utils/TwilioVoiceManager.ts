...
class TwilioVoiceManager {
  ...
  private tokenEndpoint = `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/twilio-token`; // Adjust if needed
  ...
  async initialize() {
    try {
      console.log('Fetching Twilio Token from:', this.tokenEndpoint);
      ...
      if (!response.ok) {
        ...
        console.error(`Failed to fetch token. Status: ${response.status} ${response.statusText}`, errorText);
