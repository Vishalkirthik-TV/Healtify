...
const TransmissionHistory: React.FC<{
  ...
  return (
    <div className="mt-4 rounded-lg bg-zinc-900/50 p-3 border border-white/5">
      <h4 className="mb-2 text-sm font-medium text-zinc-300">Recent Transmissions</h4>
...
const VoiceActivityDetector: React.FC<VoiceActivityDetectorProps> = ({
  ...
  const [config, setConfig] = useState<VADConfig>({
    energyThreshold: 0.03, // OPTIMIZED: Reduced sensitivity (was 0.005) to ignore noise
    conversationBreakDuration: 1.2, // OPTIMIZED: Faster response (was 1.5)
    minSpeechDuration: 0.5, // OPTIMIZED: Ignore short clicks/pops (was 0.4)
    ...
    <Card className="mx-auto w-full max-w-4xl">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="flex justify-center space-x-8">
          <div className="text-center">
            <div
              className={`inline-block h-3 w-3 rounded-full ${isConnected ? 'bg-green-500' : 'bg-zinc-700'
                }`}
            />
          ...
          <div className="text-center">
            <div
              className={`inline-block h-3 w-3 rounded-full ${isListening ? 'bg-blue-500' : 'bg-zinc-700'
                }`}
            />
          ...
          <div className="text-center">
            <div
              className={`inline-block h-3 w-3 rounded-full ${isSpeechActive ? 'bg-orange-500' : 'bg-zinc-700'
                }`}
            />
          ...
          <div className="text-center">
            <div
              className={`inline-block h-3 w-3 rounded-full ${cameraStream ? 'bg-purple-500' : 'bg-zinc-700'
                }`}
            />
