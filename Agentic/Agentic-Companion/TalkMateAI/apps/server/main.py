...
from fastapi import FastAPI, WebSocket, WebSocketDisconnect, HTTPException, Request, Response, Depends, status
...
# Configure logging
# Configure logging
# FORCE UTF-8 for Windows Console to prevent crash on Emoji/Hindi
if sys.platform == "win32":
    sys.stdout.reconfigure(encoding='utf-8')
...
class MemoryManager:
    ...
    def get_system_prompt(self, language="en"):
        ...
        if language and language != "en":
             ...
             prompt += f"\nLANGUAGE INSTRUCTION: The user is speaking in language code '{language}'. YOU MUST RESPOND IN THAT LANGUAGE ONLY.\n"
             prompt += "1. DO NOT USE ENGLISH unless the user speaks English.\n"
             prompt += "2. DO NOT TRANSLATE your response.\n"
             prompt += "3. DO NOT REPEAT the response in English.\n"
             prompt += f"4. Output ONLY the {language} response.\n\n"
...
- If the user asks to call a specific person OR number (e.g., "Call Rahul", "Call Mom", "Call 123"):
  ...
  2. Once confirmed, output EXACTLY: [CALL: +917588472102]
  3. ALWAYS use this hardcoded number +917588472102 for ANY call request.
...
- If the user wants to SHOP, BROWSE, or BUY something (e.g. "I want to buy a phone", "Search Amazon for shoes", "Look on Flipkart"):
  ...
  - Example: "Scrolling down." [ACTION: scroll_down]

- If the user wants to SET A REMINDER (e.g., "Remind me to call Mom", "Set a reminder"):
  1. If they didn't specify, ASK: "How do you want the reminder? SMS or Call?"
  2. If they didn't specify time, ASK: "What time and date?"
  3. Once you have Type (SMS/Call), Time, and Message, output command: 
     [REMINDER: type=<sms|call>, time=<ISO_8601_DATETIME>, message=<text>]
     - Example: [REMINDER: type=sms, time=2024-12-25T14:30:00, message=Call Mom]
     - Example: [REMINDER: type=call, time=2024-12-20T09:00:00, message=Wake up]
  4. Ensure the time is in the FUTURE. Today is {datetime.now().strftime("%Y-%m-%d %H:%M")}.
...
class TwilioManager:
    ...
    def send_sms(self, to_number: str, body: str):
        ...
        target = self.target_number if (not to_number or to_number == "target") else to_number
    ...
    def make_call(self, to_number: str, message: str):
        """Trigger an outbound call with a TTS message"""
        if not self.client or not self.from_number:
            logger.error("Twilio not configured for Calls")
            return False

        target = self.target_number if (not to_number or to_number == "target") else to_number

        try:
            # Use twimlets for simple TTS echo
            import urllib.parse
...
async def reminder_background_service():
    """Background task to check and send reminders"""
    logger.info("⏰ Reminder Service Started")
    twilio = TwilioManager.get_instance()
    
    while True:
        try:
            # Check every 10 seconds (responsive)
            await asyncio.sleep(10)
            
            pending = await database.get_pending_reminders()
            if pending:
                logger.info(f"⏰ Found {len(pending)} due reminders")
                ...
                for r in pending:
                    r_id = r["_id"]
                    r_type = r.get("type", "sms")
                    r_msg = r.get("message", "Reminder")
                    user_id = r["user_id"]
                    
                    # 1. VISUAL/AUDIO REMINDER (If Connected)
                    if user_id in manager.active_connections:
                         try:
                             logger.info(f"🔔 Delivering In-App Reminder to {user_id}")
                             ws = manager.active_connections[user_id]
                             
                             # Speak it!
                             speak_text = f"Excuse me. I have a reminder for you: {r_msg}"
                             audio, timings = await tts_processor.synthesize_initial_speech_with_timing(speak_text)
                             
                             if audio is not None:
                                 # Send Audio
                                 import base64
                                 # Standard audio message format used by WebSocket context
                             ...
                             logger.error(f"Error sending in-app reminder: {e}")

                    # 2. TWILIO BACKUP
                    # For now, send to default target number (user phone)
                    target_phone = os.getenv('USER_PHONE_NUMBER') 
                    
                    success = False
                    if not target_phone:
                        logger.warning(f"No phone number for reminder {r_id}")
                        # Don't fail completely if in-app worked, but update status
                        if user_id not in manager.active_connections:
                             await database.update_reminder_status(r_id, "failed_no_phone")
                             continue
                    else:
                        if r_type == "sms":
                            success = twilio.send_sms(target_phone, f"REMINDER: {r_msg}")
                        else:
                            success = twilio.make_call(target_phone, f"This is your reminder: {r_msg}")
                        
                    status = "sent" if (success or user_id in manager.active_connections) else "failed"
                    await database.update_reminder_status(r_id, status)
                    logger.info(f"Reminder {r_id} processed: {status}")
...
class GeminiProcessor:
    ...
    def __init__(self):
        ...
        # Using integer enum values to bypass potential string mapping issues
        ...
        self.model = genai.GenerativeModel(
            model_name="gemini-2.0-flash",  # Updated to valid model from list
    ...
    async def get_or_create_session(self, user_id: str, language: str = "en"):
        """Get existing session or create new one for user"""
        # Note: If language changes mid-session, we might need to update the prompt. 
        # For now, we only set it on creation.
        
        if user_id not in self.sessions:
            logger.info(f"🆕 Creating new session for user: {user_id} (Lang: {language})")
            ...
            system_prompt = memory_mgr.get_system_prompt(language=language)

            # --- LONG TERM MEMORY INJECTION ---
            try:
                recent_context = await database.get_recent_conversations(user_id, limit=3)
                if recent_context:
                    system_prompt += f"\n\n--- PREVIOUS CONVERSATION HISTORY (LONG TERM MEMORY) ---\n{recent_context}\n--- END HISTORY ---\n"
                    logger.info(f"🧠 Injected LTM context for {user_id}")
            except Exception as e:
                logger.warning(f"Failed to load LTM: {e}")
            # ----------------------------------
    ...
    async def save_session_to_ltm(self, user_id: str):
        """Save current session history to Long-Term Memory (MongoDB)"""
        if user_id in self.sessions:
            try:
                history = self.sessions[user_id].history
                # Convert glm.Content to dicts
                serializable_history = []
                for msg in history:
                    # Skip system prompt setup messages
                    if msg.role == "user" and "You are Emily" in msg.parts[0].text:
                         continue
                    if msg.role == "model" and "Understood" in msg.parts[0].text:
                         continue
                ...
                if serializable_history:
                    ...
                    try:
                        ...
                        summary_prompt = f"Summarize the following conversation in 1 short sentence (max 10 words):\n\n{conversation_text}"
                        summary_response = await self.model.generate_content_async(summary_prompt)
                        summary = summary_response.text.strip()
                    except Exception as sum_err:
                        logger.warning(f"Failed to generate summary: {sum_err}")

                    await database.save_conversation_log(user_id, serializable_history, summary=summary)
                    
                    # Clear session from memory to free space
                    del self.sessions[user_id]
                    logger.info(f"💾 Session saved (Summary: {summary}) and cleared for {user_id}")
            except Exception as e:
                logger.error(f"❌ Error saving session to LTM: {e}")
    ...
    async def generate_response(self, text: str, user_id: str, image_data: bytes = None):
        ...
        try:
            ...
            chat_session = await self.get_or_create_session(user_id)
    ...
    async def generate_response_streaming(self, text: str, user_id: str, image_data: list[bytes] | bytes = None, language: str = "en"):
        ...
        try:
            logger.info(f"Gemini streaming for {user_id}: '{text}' (Image Data: {bool(image_data)}) [Lang: {language}]")
            
            chat_session = await self.get_or_create_session(user_id, language=language)
            ...
            # Gemini SDK requires synchronous iteration - use thread with queue
            import queue
            result_queue = queue.Queue()
            
            def stream_in_thread():
                """Run Gemini streaming in thread and push chunks to queue"""
                try:
                    ...
                    logger.error(f"Error in Gemini thread: {e}")
            ...
            # Process streaming chunks from queue
            while True:
                try:
                    ...
                    if msg_type == 'error':
                        logger.error(f"Gemini stream error: {data}")
                ...
                try:
                    ...
                    if hasattr(candidate, 'finish_reason') and candidate.finish_reason:
                        ...
                        if finish_reason == 2:  # SAFETY block
                            ...
                            # Continue to try getting text or next chunk
                            pass

                    # Try to get text from chunk safely
                    text_chunk = ""
                    try:
                        ...
                        # Accessing .text on a filtered candidate raises ValueError
                        logger.debug(f"Could not extract text from filtered chunk: {e}")
                        continue
                        
                    if text_chunk:
                        chunk_text = text_chunk.replace("*", "")  # Remove markdown
                        buffer += chunk_text
...
class DeepgramProcessor:
    """Handles speech-to-text using Deepgram Nova-2 API (Primary, ultra-fast)"""
    ...
    def __init__(self):
        ...
        if not self.api_key:
            logger.warning("DEEPGRAM_API_KEY not found - will fall back to Whisper")
        
        self.api_url = "https://api.deepgram.com/v1/listen"
        logger.info("Deepgram STT processor initialized (Nova-2)")
        self.transcription_count = 0
    
    async def transcribe_audio(self, audio_bytes, language="en", **kwargs):
        """Transcribe audio using Deepgram API"""
        if not self.api_key:
            raise Exception("Deepgram API key not configured")
        
        try:
            # Audio from browser is PCM 16-bit signed int, 16000 Hz, mono
            headers = {
                "Authorization": f"Token {self.api_key}",
                "Content-Type": "audio/raw"  # Raw PCM
            }
            
            # Query params for Deepgram Nova-2
            params = {
                "model": "nova-2",
                "detect_language": "true",  # Auto-detect language (more reliable)
            ...
            # If user explicitly selected a language, use it as a hint (not forced)
            if language and language != "en":
                params["language"] = language  # Use as hint, not absolute requirement
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status != 200:
                        ...
                        raise Exception(f"Deepgram API failed: {response.status}")
                    ...
                    logger.info(f"Deepgram transcription #{self.transcription_count} ({elapsed:.2f}s): '{transcript}'")
                    
                    # Check for noise/empty
...
class WhisperProcessor:
    ...
    def __init__(self):
        ...
        # Load Whisper Tiny (Multilingual) as Fallback
        model_id = "openai/whisper-tiny"
        logger.info(f"Loading Whisper Fallback: {model_id}...")
    ...
    async def transcribe_audio(self, audio_bytes, **kwargs):
        ...
        try:
            ...
            result = await asyncio.get_event_loop().run_in_executor(
                None, lambda: self.pipe(audio_array, **kwargs)
            )
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        reminder_task = asyncio.create_task(reminder_background_service())
...
app.add_middleware(
    ...
    allow_origins=[
        "*", 
        "https://mississippi-capabilities-opposition-authorities.trycloudflare.com",
        "https://5301dd0e4888.ngrok-free.app",
        "http://localhost:3000"
    ],
...
async def register(user: UserCreate):
    ...
    user_id = await database.create_user(user.username, hashed_password)
...
@app.post("/token", response_model=auth_utils.Token)
...
@app.get("/users/me", response_model=auth_utils.User)
async def read_users_me(current_user: auth_utils.User = Depends(auth_utils.get_current_user)):
    return current_user
...
@app.get("/reminders")
async def get_my_reminders(current_user: auth_utils.User = Depends(auth_utils.get_current_user)):
    """Get reminders for the current user"""
    try:
        reminders = await database.get_user_reminders(current_user.username)
...
async def get_twilio_token():
    ...
    try:
        ...
        # Use provided env var names
        api_key = os.getenv("TWILIO_API_KEY_SID") or os.getenv("TWILIO_API_KEY") 
        api_secret = os.getenv("TWILIO_API_KEY_SECRET") or os.getenv("TWILIO_API_SECRET")
        ...
        # Create access token
        identity = "browser_user"  # Unique ID for the browser client
        ...
        logger.info(f"Generated Twilio Token for {identity}")
...
async def get_my_conversations(current_user: auth_utils.User = Depends(auth_utils.get_current_user)):
    """Get conversation history for the current user"""
    try:
        conversations = await database.get_all_conversations(current_user.username)
...
async def websocket_endpoint(websocket: WebSocket, client_id: str):
    ...
    deepgram_processor = DeepgramProcessor.get_instance()  # Primary STT
    whisper_processor = WhisperProcessor.get_instance()  # Fallback STT
    ...
    try:
        # Initialize session for this user (load memory + LTM)
        chat_session = await gemini_processor.get_or_create_session(client_id)
        ...
        async def process_audio_segment(audio_data=None, image_data=None, text_input=None):
            ...
            try:
                ...
                if audio_data:
                    # Get language preference from client config
                    client_config = manager.get_config(client_id)
                    language = client_config.get("language", "en")  # Default: English
                    
                    # Try Deepgram first (Primary, ultra-fast)
                    transcribed_text = None
                    try:
                        logger.info(f"Starting Deepgram transcription (lang={language})")
                        transcribed_text = await deepgram_processor.transcribe_audio(audio_data, language=language)
                        logger.info(f"Deepgram transcription result: '{transcribed_text}'")
                    except Exception as dg_error:
                        logger.warning(f"Deepgram failed: {dg_error}. Falling back to Whisper...")
                        # Fallback to Whisper
                        try:
                            logger.info(f"Starting Whisper transcription (fallback)")
                            transcribed_text = await whisper_processor.transcribe_audio(audio_data, return_timestamps=True)
                            logger.info(f"Whisper transcription result: '{transcribed_text}'")
                        except Exception as w_error:
                            logger.error(f"Whisper also failed: {w_error}")
                            transcribed_text = None
                else:
                    transcribed_text = text_input

                # Guard Clause: If we have no text, no audio, and no explicit instruction, abort.
                if not transcribed_text:
                    logger.warning("No input content to process (No audio, no text). Aborting.")
                    return
                ...
                if not image_data:
                    ...
                    if screenshot_b64:
                        logger.info("Injecting Browser Screenshot into Gemini Context")
                ...
                config = manager.get_config(client_id)
                ...
                async for sentence in gemini_processor.generate_response_streaming(
                    contextualized_text, client_id, image_data, language=language
                ):
                    sentence_count += 1
                    ...
                    if sentence and sentence.strip():
                        ...
                        if "[CALL:" in sentence:
                            ...
                            try:
                                ...
                                # [ACTION: scroll_down]
                                action_match = re.search(r'\[ACTION:\s*([\w_]+)(?:\s+(.+?))?\]', sentence)
                                if action_match:
                                    action_type = action_match.group(1).lower()
                                    params = action_match.group(2).strip() if action_match.group(2) else ""
                                    
                                    logger.info(f"Executing Browser Action: {action_type} on {params}")
                                    
                                    if action_type == "click":
                                        # Clean quotes from params just in case
                                        params = params.strip('"\'')
                                        ...
                                        if "down" in action_type or params == "down":
                                            await browser_manager.run_action("scroll_down")
                                        else:
                                            await browser_manager.run_action("scroll_up")
                                            
                                    # Remove command from spoken text so Avatar doesn't say it
                                    sentence = sentence.replace(action_match.group(0), "").strip()
                            ...
                            try:
                                logger.info(f"⏰ REMINDER command detected: {sentence}")
                                # Pattern: [REMINDER: type=sms, time=2024..., message=...]
                                # Robust parsing: split by commas, then key=value
                                content = sentence[sentence.find("[REMINDER:") + 10 : sentence.find("]")]
                                ...
                                for p in parts:
                                    if "=" in p:
                                        ...
                                        v = v.strip()
                                        if k == "type": r_type = v
                                        elif k == "time": r_time = v
                                        elif k == "message": r_msg = v
                                
                                if r_time:
                                    success = await database.save_reminder(client_id, r_type, r_time, r_msg)
                                    if success:
                                        logger.info(f"✅ Reminder set: {r_type} at {r_time}")
                                        # Provide verbal confirmation (replace command with text)
                                        sentence = f"I've set a {r_type} reminder for that time."
                                    else:
                                         sentence = "I couldn't set the reminder due to a database error."
                                else:
                                    logger.warning("Reminder command missing time")
            ...
            try:
                while True:
                    ...
                    try:
                        ...
                        # Handle Config Updates (Language, Mode)
                        elif message.get("type") == "config":
                            logger.info(f"Received config update from {client_id}: {message}")
                            manager.update_config(client_id, message)
        ...
        # Cleanup
        logger.info(f"Cleaning up resources and saving memory for client {client_id}")
        # SAVE MEMORY TO LTM
        try:
            gemini_processor = GeminiProcessor.get_instance()
            await gemini_processor.save_session_to_ltm(client_id)
        except Exception as e:
            logger.error(f"Error saving LTM: {e}")
