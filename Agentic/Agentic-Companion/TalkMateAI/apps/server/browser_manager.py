...
class BrowserManager:
    ...
    async def click_element(self, selector: str):
        ...
        try:
            ...
            # Helper to execute click with retries
            def _do_click_sync():
                try:
                    # 1. Try as CSS selector
                    self.page_sync.click(selector, timeout=2000)
                except Exception:
                    try:
                        # 2. Try as exact text
                        logger.info(f"CSS failed, trying text='{selector}'")
                        self.page_sync.click(f"text={selector}", timeout=2000)
                    except Exception:
                        # 3. Try as partial link text
                        logger.info(f"Text failed, trying link name='{selector}'")
                        self.page_sync.get_by_role("link", name=selector).first.click(timeout=2000)

                import time
                time.sleep(1)

            async def _do_click_async():
                try:
                    await self.page_async.click(selector, timeout=2000)
                except Exception:
                    try:
                        await self.page_async.click(f"text={selector}", timeout=2000)
                    except Exception:
                        await self.page_async.get_by_role("link", name=selector).first.click(timeout=2000)
                await asyncio.sleep(1)

            if self.use_sync_api:
                await asyncio.get_event_loop().run_in_executor(self._executor, _do_click_sync)
            else:
                await _do_click_async()
