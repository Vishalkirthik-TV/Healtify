...
import json
import scipy.io.wavfile as wavfile  # For WAV encoding (Robust Audio)
...
class GeminiProcessor:
    ...
    def __init__(self):
        ...
        # Using gemini-1.5-flash for speed and low latency
        generation_config = {
            "temperature": 0.9,  
            "top_p": 0.95,
            "top_k": 40,
        ...
        # COMPREHENSIVE SAFETY SETTINGS - DISABLE ALL FILTERS
        # Using STRING keys to avoid Enum versioning issues
        safety_settings = [
...
class EdgeTTSProcessor:
    """Handles text-to-speech conversion using Microsoft Edge TTS (Cloud)"""
    ...
    def __init__(self):
        logger.info("Initializing Edge TTS processor...")
        self.voice = "en-US-AriaNeural" # or en-US-ChristopherNeural
        self.rate = "+0%"
        self.pitch = "+0Hz"
        self.synthesis_count = 0
        logger.info(f"Edge TTS processor initialized (Voice: {self.voice})")

    async def synthesize_initial_speech_with_timing(self, text):
        """Convert text to speech using Edge TTS and extract timing"""
        if not text:
            return None, []

        try:
            logger.info(f"Synthesizing speech with Edge TTS: '{text}'")
            ...
            async for chunk in communicate.stream():
                if chunk["type"] == "audio":
                    ...
                    # duration is in 100ns units (ticks), offset is in 100ns units
            ...
            data, samplerate = sf.read(audio_io) # Decodes MP3 to Float32
            
            self.synthesis_count += 1
            
            logger.info(f"✨ Edge TTS complete: {len(data)} samples, {len(timings)} word timings, {samplerate}Hz")
    ...
    # Alias for compatibility if needed, but we only use one main method now
    async def synthesize_remaining_speech_with_timing(self, text):
        return await self.synthesize_initial_speech_with_timing(text)
...
class ConnectionManager:
    def __init__(self):
        ...
        # Track current tasks to allow cancellation
        self.current_tasks: dict[str, dict] = {}
        # Track session state per user (sequence IDs, etc.)
    ...
    def disconnect(self, client_id: str):
        ...
        # We don't delete the session to persist seq_id across minor reconnects, 
        # or we could: if client_id in self.sessions: del self.sessions[client_id]
        logger.info(f"Client {client_id} disconnected")

    def get_next_seq_id(self, client_id: str) -> int:
        """Increment and return the next sequence ID for a client"""
        if client_id not in self.sessions:
            self.sessions[client_id] = {"seq": 0}
        self.sessions[client_id]["seq"] += 1
        return self.sessions[client_id]["seq"]

    def get_current_seq_id(self, client_id: str) -> int:
        """Get the current sequence ID without incrementing"""
        return self.sessions.get(client_id, {}).get("seq", 0)
    ...
    def set_task(self, client_id: str, task_type: str, task: asyncio.Task):
        ...
        if client_id not in self.current_tasks:
            self.current_tasks[client_id] = {}
        self.current_tasks[client_id][task_type] = task

    async def cancel_current_tasks(self, client_id: str):
        """Cancel all running tasks for a client (audio processing, TTS, etc.)"""
        if client_id in self.current_tasks:
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        tts_processor = EdgeTTSProcessor.get_instance()
        logger.info("All models initialized successfully (using Gemini 1.5 Flash + Memory + Edge TTS)")
    except Exception as e:
                ...
                async for sentence in gemini_processor.generate_response_streaming(
                    ...
                    if sentence and sentence.strip():
                        ...
                        if audio is not None and len(audio) > 0:
                            # Convert Float32 (-1.0 to 1.0) to Int16 (-32767 to 32767)
                            ...
                            audio_message = {
                                ...
                                "sample_rate": 24000,
                                "method": "wav_container", # Signaling the change
                                "modality": "multimodal" if image_data else "audio_only",
                                "sentence_number": sentence_count,
                                "streaming": True,
                                "seq": seq_id, # INCLUDE SEQUENCE ID
                            }

                            await websocket.send_text(json.dumps(audio_message))
                            logger.info(
                                f"🎤 Sentence {sentence_count} WAV sent (seq={seq_id}, {len(wav_bytes)} bytes)"
                            )
                ...
                while True:
                    ...
                    try:
                        ...
                        if "audio_segment" in message:
                            ...
                            # Increment sequence ID for new turn
                            seq_id = manager.get_next_seq_id(client_id)
                            ...
                            if "image" in message:
                                ...
                                logger.info(
                                    f"Received audio+image (seq={seq_id}): audio={len(audio_data)} bytes, image={len(image_data)} bytes"
                                )
                            else:
                                logger.info(
                                    f"Received audio-only (seq={seq_id}): {len(audio_data)} bytes"
                                )

                            # Start processing the audio segment with optional image
                            processing_task = asyncio.create_task(
                                process_audio_segment(audio_data, image_data, seq_id)
                            )
                            manager.set_task(client_id, "processing", processing_task)

                        # Handle standalone images (only if not currently processing)
                        elif "type" in message and message["type"] == "interrupt":
                             logger.info(f"🛑 Received manual interrupt signal from {client_id}")
                             await manager.cancel_current_tasks(client_id)
                             
                             # Increment sequence ID to invalidate any pending audio
                             seq_id = manager.get_next_seq_id(client_id)
                             # Send interrupt confirmation with new seq_id
                             await manager.send_personal_message(
                                 json.dumps({"type": "interrupt", "seq": seq_id}), 
                                 client_id
                             )
                        ...
                        # Handle realtime input (for backward compatibility)
                            ...
                            for chunk in message["realtime_input"]["media_chunks"]:
                                if chunk["mime_type"] == "audio/pcm":
                                    ...
                                    processing_task = asyncio.create_task(
                                        process_audio_segment(audio_data, None, seq_id)
                                    )
                                    manager.set_task(
